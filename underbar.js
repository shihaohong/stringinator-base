...
const isArrayLike = function(obj) {
  // should have a length property, equal to a number, and greater than or equal to 0
...
const each = function(obj, callback=identity) {
  if (isArrayLike(obj)){ // case for array-like objects
    for (let index = 0; index < obj.length; index++){
      callback(obj[index], index, obj);
    }
  } else { // case for non-array-like objects
    for (let key in obj) {
...
const pluck = function(arr, key) {
  return map(arr, item => item[key]);
}

// Reduces collection to a value which lists the accumulated result of running
